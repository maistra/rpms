...
function run_build() {
  pushd ${RPM_BUILD_DIR}/${PROXY_NAME}-${PROXY_GIT_BRANCH}/${PROXY_NAME}/proxy
    ...
    RECIPES_DIR=${RPM_BUILD_DIR}/${PROXY_NAME}-${PROXY_GIT_BRANCH}/${PROXY_NAME} bazel --output_base=${RPM_BUILD_DIR}/${PROXY_NAME}-${PROXY_GIT_BRANCH}/${PROXY_NAME}/bazel/base --output_user_root=${RPM_BUILD_DIR}/${PROXY_NAME}-${PROXY_GIT_BRANCH}/${PROXY_NAME}/bazel/root build --override_repository=envoy=${ENVOY_CACHE_DIR} --jobs=3 --force_pic=true --config=${BUILD_CONFIG} --sandbox_debug=true --force_pic=true --config=${BUILD_CONFIG} --host_javabase=@local_jdk//:jdk "//..."
